...
- This is because the samples were randomly generated from
  x∼Normal\[μ,σ\], and when the number of samples (and null rejected
  samples) is large enough, their mean distribution tend to be
  approximately **symmetric** to the true μ due to the randomness,
  causing the average of null rejected samples’ $\hat{μ}$ to be
  approximately equal to μ.
